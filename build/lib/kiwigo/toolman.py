...
class Toolman():
    ...
    # 一个添加dict的神器
    def dicter(self, d0, k1, k, v):
        if k1 not in d0.keys() or (type(d0[k1]).__name__!='dict'):
            d0[k1] = {}
        d0[k1][k] = v

    # zip解压去除乱码辅助工具
    def _zip_support_gbk(self, zip_file: zipfile.ZipFile):
    ...
    # todo: 解压工具
    def uncps(self, pth_f, dir_ot, pwd=''):
        # todo: 支持使用解压密码
        if not os.path.exists(dir_ot):
            os.makedirs(dir_ot)
        if pth_f.endswith('.rar'):
            try:
                with rarfile.RarFile(pth_f, 'r') as fp:
                    fp.extractall(dir_ot, pwd=pwd)
                print('[解压成功]', pth_f)
                return 0
            except:
                print('[error] Cannot uncompress this .rar file', pth_f)
                return -1
        elif pth_f.endswith('.zip'):
            try:
                with self._zip_support_gbk(zipfile.ZipFile(pth_f, mode='r')) as fp:
                    fp.extractall(dir_ot, pwd=pwd.encode('ascii'))
                print('[解压成功]', pth_f)
                return 0
            except:
                print('[error] Cannot uncompress this .zip file', pth_f)
                return -1
        elif pth_f.endswith('.7z'):
            try:
                with py7zr.SevenZipFile(pth_f, 'r', password=pwd) as fp:
                    fp.extractall(path=dir_ot)
                print('[解压成功]', pth_f)
                return 0
            except:
                print('[error] Cannot uncompress this .7z file', pth_f)
                return -1

    # 带自动命名功能的shutil.copy mode='a'(新增命名), 'w'(覆盖)
    def dup_judge(self, dst, ):
        dir_dst, fn_exe = os.path.split(dst)
        fn, _exe = os.path.splitext(fn_exe)
        lst_dir_fn = [fi for fi in os.listdir(dir_dst)]
        ...
        if os.path.exists(dst):
            ...
            while True:
                sig = ' (' + str(id) + ')'
                fn_id_exe = sig.join([fn, _exe])
                pth_fn = os.path.join(dir_dst, fn_id_exe)
